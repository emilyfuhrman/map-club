...
![U.S. Census Data 2015 Population Estimates](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/02_Dataset_2015_Population_Estimates.png)
...
![U.S. Census Data State Geography](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/03_Geography_US_States.png)
...
![U.S. Census Data Population Estimates Table](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/04_Population_Estimates_Table.png)
...
![U.S. Census Data Population Estimates Download](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/05_Population_Estimates_Table_Full.png)
...
![U.S. Census Data Population Estimates Excel](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/06_Population_Data_Excel.png)
...
* In order to create our map, the only data we need is state name, state ID, and population count. 
	...
	* Next, delete the second row of the spreadsheet, which contains descriptions for the columns. 
	...
	![Editing Population Estimates](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/07_Population_Data_Editing.png)
...
* We will be using [FIPS region codes](https://en.wikipedia.org/wiki/List_of_FIPS_region_codes_(S%E2%80%93U)#US:_United_States) to join the Natural Earth vector boundaries with the Census population data. The value in the `ID` column is a concatenated string that combines the Census table ID with individual state FIPS codes. We need to separate out the FIPS code portion of this ID so that this dataset can match up with the Natural Earth dataset, which already uses FIPS codes to identify each state. To do that, we need to create a new column that pulls *only the last four characters* in the `ID` column. We will use the `Right` function in Excel to do this.
	...
	* In the first cell of the column, type the formula `=RIGHT(A2,4)`. This will pull the last four characters in the `ID` column, `A4`, into the new `FIPS_ID` column.
	...
	![Excel FIPS Formula](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/08_Excel_FIPS_Formula.png)
	...
	* After typing the formula, hit `Enter`. Double-click on the bottom right corner of the cell to populate the entire column with the new formula.
	...
	![Excel Populated FIPS Column](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/09_Excel_Populated_FIPS_Column.png)
	
	* Name the file `StatePopulations.csv`, and save it to the a `Windows Comma Separated` format.

	![Excel Save New CSV](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/10_Excel_Save_New_CSV.png)
...
* As in Tutorial 03, we need to create a file that describes the data types in this new CSV before attempting to import it into QGIS. We will therefore use a text editor to create a `.csvt` file.
	...
	* Save the file with the same name as the CSV file, only with the `.csvt` extension: `StatePopulations.csvt`. It should look something like this:
	...
	![Create CSVT file](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/11_Population_CSVT.png)
...
![Layer Projection Metadata](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/12_Layer_Projection_Metadata.png)
...
![Select U.S. States](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/13_Select_US.png)

* Hit `Select` and close the query builder. Navigating back to the map, only the U.S. should be selected.

![Selected U.S. States](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/14_Selected_US.png)
...
![Set CRS to Albers Projection](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/15_CRS_Albers.png)
...
![Check U.S. States Metadata](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/16_Check_US_Metadata.png)
...
![U.S. States Albers Projection](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/17_Check_US_Albers.png)
...
![FIPS Alt Value for Minnesota](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/18_MN_FIPS_Alt.png)
...
![Edit FIPS Alt Value for Minnesota](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/19_Editing_MN_FIPS.png)
...
![Import Populations CSV](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/20_Import_CSV.png)
...
![Join CSV to the U.S. Albers Layer](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/21_Join_CSV_and_Albers.png)
...
![Population Map](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/22_Population_Map.png)
...
![Final Map Example](https://github.com/emilyfuhrman/map-club/blob/master/2016_Summer/Session_03/Images/23_Final_Map_Example.png)